...
class SupabaseDB:
    ...
    def _get_paginated(
        ...
        *,
        ...
        while True:
            batch_size = min(page_size, target - len(rows)) if target is not None else page_size
            if batch_size <= 0:
                break
            p = dict(params or {})
            p["limit"] = batch_size
...
class Database:
    ...
    def init(self, mode: Optional[str] = None):
        selected_mode = (mode or DB_MODE or "").strip().lower()
        if selected_mode not in {"supabase_rest", "postgres", "auto"}:
            selected_mode = "supabase_rest" if USE_SUPABASE else "postgres"
        if USE_SUPABASE and mode is None and selected_mode == "postgres":
            selected_mode = "supabase_rest"

        if selected_mode == "supabase_rest":
            self.init_supabase_rest()
        ...
        try:
            ...
            logger.warning("Supabase REST init failed in auto mode: %s", supabase_exc)
            self.init_postgres()

    def init_supabase_rest(self) -> None:
        sb = SupabaseDB()
        sb.init()
        self._sb = sb
        self._pg = None
    ...
    def _fetch_pg_dicts(self, sql: str, params: tuple = ()) -> List[Dict[str, Any]]:
        if not self._pg:
            return []
        with self._pg._get_conn() as conn:
            with conn.cursor() as cur:
                cur.execute(sql, params)
    ...
    @staticmethod
    def _ml_cutoff_iso(months: int) -> str:
        from datetime import timedelta

        cutoff = datetime.now(timezone.utc) - timedelta(days=max(1, int(months)) * 30)
        return cutoff.isoformat()

    def fetch_analyst_trade_reviews_for_ml(self, symbol: str, months: int, limit: Optional[int] = 50000) -> List[Dict[str, Any]]:
        cutoff = self._ml_cutoff_iso(months)
        if self._sb:
            rows = self._sb._get_paginated(
                "analyst_trade_reviews",
                {
                    "symbol": f"eq.{symbol}",
        ...
        if self._pg:
            return self._fetch_pg_dicts(
                """
                SELECT *
                FROM analyst_trade_reviews
                WHERE symbol = %s
                  AND created_at >= %s
    ...
    def fetch_analyst_confirmations_for_ml(self, symbol: str, months: int, limit: Optional[int] = 50000) -> List[Dict[str, Any]]:
        cutoff = self._ml_cutoff_iso(months)
        if self._sb:
            rows = self._sb._get_paginated(
                "analyst_confirmation_history",
        ...
        if self._pg:
            return self._fetch_pg_dicts(
                """
                SELECT *
                FROM analyst_confirmation_history
                WHERE symbol = %s
                  AND created_at >= %s
                ORDER BY created_at ASC
                LIMIT %s
    ...
    def fetch_analyst_scenarios_for_ml(self, symbol: str, months: int, limit: Optional[int] = 50000) -> List[Dict[str, Any]]:
        cutoff = self._ml_cutoff_iso(months)
        if self._sb:
            rows = self._sb._get_paginated(
                "analyst_scenario_history",
