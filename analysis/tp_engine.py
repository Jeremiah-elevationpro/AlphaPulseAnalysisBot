...
def _score_lookup(level_scores: dict | list | None) -> dict[float, dict]:
    if not level_scores:
        return {}
    if isinstance(level_scores, dict):
        items = level_scores.get("level_scores", level_scores.values())
    else:
        items = level_scores
    out: dict[float, dict] = {}
    for item in items or []:
        try:
            ...
            out[round(float(row.get("level")), 2)] = row
        except Exception:
            continue
    return out


def _level_allowed_by_intelligence(level: float, lookup: dict[float, dict], *, entry: float, allow_reaction: bool = False) -> tuple[bool, str, float]:
    if not PREFER_LEVEL_SCORE_FOR_TP or not lookup:
        return True, "level_intelligence_unavailable", 70.0
    row = lookup.get(round(float(level), 2))
    if not row:
        return True, "unscored_level_allowed", 70.0
    score = float(row.get("score", 0.0) or 0.0)
    state = str(row.get("state", "active"))
    use = str(row.get("recommended_use", "target"))
    if state == "consumed" and use not in {"target", "reaction_level"}:
        return False, "consumed", score
    if score < MIN_TP_LEVEL_SCORE and not (allow_reaction and use == "reaction_level"):
        return False, "weak", score
    if abs(float(level) - float(entry)) < MIN_TP1_DISTANCE_PIPS and use != "reaction_level":
        return False, "too_close", score
    return True, "score_ok", score
...
def build_structure_tps(
    ...
    sl: float | None = None,
    trade_path=None,
    level_scores: dict | list | None = None,
) -> dict[str, object]:
    """
    ...
    intelligence_lookup = _score_lookup(level_scores)
    for level in candidates:
        ...
        allowed_by_level, level_reason, level_score = _level_allowed_by_intelligence(level, intelligence_lookup, entry=entry, allow_reaction=True)
        if not allowed_by_level:
            logger.info("TP LEVEL SKIPPED: level=%.2f score=%.1f reason=%s", level, level_score, level_reason)
            continue
        ...
        logger.info("TP LEVEL SELECTED: level=%.2f score=%.1f reason=%s", level, level_score, level_reason)
...
def filter_market_plan_targets(
    ...
    *,
    ...
    lookup = _score_lookup(level_scores)
    scored_filtered: list[float] = []
    for level in pre_filtered:
        allowed, reason, score = _level_allowed_by_intelligence(level, lookup, entry=reference_entry, allow_reaction=False)
        if allowed:
            logger.info("TP LEVEL SELECTED: level=%.2f score=%.1f reason=%s", level, score, reason)
            scored_filtered.append(level)
        else:
            logger.info("TP LEVEL SKIPPED: level=%.2f score=%.1f reason=%s", level, score, reason)

    targets, _ = select_ladder_targets(
        direction=direction,
        reference=reference_entry,
        candidates=scored_filtered or pre_filtered,
        mode=mode,
        max_count=max_count,
