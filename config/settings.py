...
SCENARIO_UPDATE_ALERT_COOLDOWN_MINUTES = int(os.getenv("SCENARIO_UPDATE_COOLDOWN_MINUTES", os.getenv("SCENARIO_UPDATE_ALERT_COOLDOWN_MINUTES", "30")))
# Refinement updates (zone shifted by a few pips, same direction) get a longer
# cooldown so we never spam "scenario invalidated" for cosmetic zone moves.
SCENARIO_REFINEMENT_COOLDOWN_MINUTES = int(os.getenv("SCENARIO_REFINEMENT_COOLDOWN_MINUTES", "60"))
# Zone movement classification thresholds (pips, XAUUSD).
SCENARIO_NO_CHANGE_ZONE_TOLERANCE_PIPS = float(os.getenv("SCENARIO_NO_CHANGE_ZONE_TOLERANCE_PIPS", "10"))
SCENARIO_REFINEMENT_ZONE_TOLERANCE_PIPS = float(os.getenv("SCENARIO_REFINEMENT_ZONE_TOLERANCE_PIPS", "20"))
# Round zones to nearest N pips when building dedupe keys so tiny moves don't
# generate a fresh "new" scenario fingerprint.
SCENARIO_DEDUPE_ROUND_PIPS = int(os.getenv("SCENARIO_DEDUPE_ROUND_PIPS", "5"))
# Buffer used when deciding whether current price has truly closed beyond an
# invalidation level / zone (avoids classifying a tag/wick as a reclaim).
SCENARIO_RECLAIM_BUFFER_PIPS = float(os.getenv("SCENARIO_RECLAIM_BUFFER_PIPS", "0.5"))
ACTIVE_SCENARIO_MAX_DISTANCE_PIPS = float(os.getenv("ACTIVE_SCENARIO_MAX_DISTANCE_PIPS", "80"))
ACTIVE_SCENARIO_IDEAL_MIN_DISTANCE_PIPS = float(os.getenv("ACTIVE_SCENARIO_IDEAL_MIN_DISTANCE_PIPS", "5"))
ACTIVE_SCENARIO_IDEAL_MAX_DISTANCE_PIPS = float(os.getenv("ACTIVE_SCENARIO_IDEAL_MAX_DISTANCE_PIPS", "50"))
DEEP_CONTEXT_DISTANCE_PIPS = float(os.getenv("DEEP_CONTEXT_DISTANCE_PIPS", "80"))

# ─────────────────────────────────────────────
# SESSION LIQUIDITY INTELLIGENCE (Gold-specific)
# ─────────────────────────────────────────────
SESSION_LIQUIDITY_ENABLED = os.getenv("SESSION_LIQUIDITY_ENABLED", "true").strip().lower() == "true"
SESSION_LIQUIDITY_BLOCKING_MODE = os.getenv("SESSION_LIQUIDITY_BLOCKING_MODE", "false").strip().lower() == "true"
SESSION_LIQUIDITY_ADVISORY_MODE = os.getenv("SESSION_LIQUIDITY_ADVISORY_MODE", "true").strip().lower() == "true"
# Session boundaries are stored as HH:MM strings in 24-hour broker time. The
# engine reads broker server timestamps from the OHLCV feed directly, so the
# offset below is for logging/UI display only.
ASIA_SESSION_START = os.getenv("ASIA_SESSION_START", "00:00")
ASIA_SESSION_END = os.getenv("ASIA_SESSION_END", "07:00")
LONDON_SESSION_START = os.getenv("LONDON_SESSION_START", "07:00")
LONDON_SESSION_END = os.getenv("LONDON_SESSION_END", "13:00")
NEW_YORK_SESSION_START = os.getenv("NEW_YORK_SESSION_START", "13:00")
NEW_YORK_SESSION_END = os.getenv("NEW_YORK_SESSION_END", "21:00")
LONDON_NY_OVERLAP_START = os.getenv("LONDON_NY_OVERLAP_START", "13:00")
LONDON_NY_OVERLAP_END = os.getenv("LONDON_NY_OVERLAP_END", "16:00")
BROKER_TIMEZONE_OFFSET = int(os.getenv("BROKER_TIMEZONE_OFFSET", "0"))
SESSION_TIMEZONE = os.getenv("SESSION_TIMEZONE", "UTC")
# Liquidity scoring / sweep thresholds.
SESSION_LIQUIDITY_MIN_LEVEL_SCORE = float(os.getenv("SESSION_LIQUIDITY_MIN_LEVEL_SCORE", "70"))
SESSION_LIQUIDITY_EQUAL_HIGH_TOLERANCE_PIPS = float(os.getenv("SESSION_LIQUIDITY_EQUAL_HIGH_TOLERANCE_PIPS", "1.5"))
SESSION_LIQUIDITY_SWEEP_MIN_WICK_PIPS = float(os.getenv("SESSION_LIQUIDITY_SWEEP_MIN_WICK_PIPS", "1.0"))
SESSION_LIQUIDITY_RECLAIM_BUFFER_PIPS = float(os.getenv("SESSION_LIQUIDITY_RECLAIM_BUFFER_PIPS", "0.5"))
SESSION_LIQUIDITY_DISPLACEMENT_MIN_PIPS = float(os.getenv("SESSION_LIQUIDITY_DISPLACEMENT_MIN_PIPS", "5.0"))
...
PYTORCH_AI_ENABLED = os.getenv("PYTORCH_AI_ENABLED", "true").strip().lower() == "true"
PYTORCH_AI_BLOCKING_MODE = os.getenv("PYTORCH_AI_BLOCKING_MODE", "false").strip().lower() == "true"
PYTORCH_AI_MODEL_PATH = os.getenv("PYTORCH_AI_MODEL_PATH", "data/ml/models/setup_approved_outcome_model_v1.pt").strip()
PYTORCH_AI_SCHEMA_PATH = os.getenv("PYTORCH_AI_SCHEMA_PATH", "data/ml/approved_feature_schema.json").strip()
PYTORCH_AI_MODEL_TYPE = os.getenv("PYTORCH_AI_MODEL_TYPE", "approved_setup_outcome").strip()
PYTORCH_AI_MODEL_VERSION = os.getenv("PYTORCH_AI_MODEL_VERSION", "setup_approved_outcome_model_v1").strip()
PYTORCH_MIN_TP1_PROBABILITY_TO_BOOST = float(os.getenv("PYTORCH_MIN_TP1_PROBABILITY_TO_BOOST", "0.70"))
PYTORCH_MIN_TP1_PROBABILITY_TO_ALLOW = float(os.getenv("PYTORCH_MIN_TP1_PROBABILITY_TO_ALLOW", "0.55"))
PYTORCH_MAX_SL_PROBABILITY = float(os.getenv("PYTORCH_MAX_SL_PROBABILITY", "0.35"))
PYTORCH_AI_DEBUG_FEATURES = os.getenv("PYTORCH_AI_DEBUG_FEATURES", "false").strip().lower() == "true"
PYTORCH_AI_NEUTRAL_TP1 = float(os.getenv("PYTORCH_AI_NEUTRAL_TP1", "0.55"))
PYTORCH_AI_NEUTRAL_SL = float(os.getenv("PYTORCH_AI_NEUTRAL_SL", "0.30"))
PYTORCH_AI_MIN_CRITICAL_COMPLETENESS = float(os.getenv("PYTORCH_AI_MIN_CRITICAL_COMPLETENESS", "0.7"))
...
# DATABASE
# ─────────────────────────────────────────────
DB_MODE = os.getenv("DB_MODE", "supabase_rest").strip().lower()
...
SCENARIO_COMPLIANCE_ENABLED = os.getenv("SCENARIO_COMPLIANCE_ENABLED", "true").lower() == "true"
MIN_MAIN_TARGET_DISTANCE_PIPS = float(os.getenv("MIN_MAIN_TARGET_DISTANCE_PIPS", "10"))
PREFER_LEVEL_SCORE_FOR_TP = os.getenv("PREFER_LEVEL_SCORE_FOR_TP", "true").lower() == "true"
MIN_TP_LEVEL_SCORE = float(os.getenv("MIN_TP_LEVEL_SCORE", "65"))
